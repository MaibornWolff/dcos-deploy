...
class EdgeLbAdapter(object):
    ...
    def get_pools(self):
        ...
        response = http.get(self.base_url + "v2/pools")
        if not response.ok:
    ...
    def get_pool(self, name):
        ...
        response = http.get(self.base_url + "v2/pools/%s" % name)
        if not response.ok:
            echo_error(response.text)
    ...
    def create_pool(self, config):
        """Create a new pool"""
        response = http.post(self.base_url + "v2/pools", json=config)
        if not response.ok:
    ...
    def update_pool(self, config):
        ...
        response = http.put(self.base_url + "v2/pools/%s" % config["name"], json=config)
        if not response.ok:
            raise Exception("Could not update pool %s: %s " % (config["name"], response.text))

    def delete_pool(self, name):
        response = http.delete(self.base_url + "v2/pools/%s" % name)
        if response.ok:
            return True
    ...
    def ping(self):
        response = http.get(self.base_url + "ping")
        if not response.ok:
